...
class Client(object):
    def __init__(self,
                 ...
                 seq_bits:int=16,
                 window_size:int=1,
                 client_data_folder:str=os.path.join(os.getcwd(), "Client/downloads"),file_name:str="",timeout:int=10,count:int=None):
        ...
        self.thread=threading.Thread(target=self.receive,args=[file_name,server_ip,server_port,client_ip, client_port,timeout,count])
    ...
    def receive(self,
        ...
        self.client_socket.sendto(file_name.encode(), (self.server_ip, self.server_port))
        ...
        packet_handler = PacketHandler(self.file_handle, self.client_socket, server_ip,
                                       server_port, self.client_ip, self.client_port, window, timeout,count=count)
...
class PacketHandler(Thread):
    ...
    def __init__(self,
                 ...
                 window,
                 timeout=10,
                 buffer_size=32736,count:Iclient=None):
            ...
            if not ready[0]:
                if not self.window.receipt():
                    ...
                    if chance == 1:
                        ...
                        self.count.download_file_count_pack=0
                        break
                    else:
                        chance += 1
                        self.count.download_file_count_pack+=1
                        continue
            else:
                self.count.download_file_count_pack=self.count.download_file_count_pack-1 if self.count.download_file_count_pack-1!=0 else self.count.download_file_count_pack
                chance = 0
                if not self.window.receipt():
    ...
    def write_received_data_to_file(self):
        while True:
            packet:PacketHandler.PACKET = next(self.window)
...
def create_packet(ack:PacketHandler.PACKET):
...
def is_corrupt(receivedPacket:PacketHandler.PACKET):
